...
public class Taicho implements ApplicationListener {
	...
	public static final int VIRTUAL_WIDTH = 1200;//1380;
	public static final int VIRTUAL_HEIGHT = 720;//820;
...
//		_states.put("StateHowto", new StateHowto(this));
	...
	private void performPendingStateChange() {
		...
		if (_nextState != null) {
			...
			if( _currentState instanceof StateMenu ){
				
			}else if( _currentState instanceof StateGame ){
				_currentState = (StateGame) _currentState;
				gamewinner = ((StateGame) _currentState).getGameWinner();
			}
...
//			if( gamewinner != Player.NONE && _currentState instanceof StateGameOverMenu){	//GameState is no longer current, game is over show menu with winner
			if( _currentState instanceof StateGameOverMenu){	//GameState is no longer current, game is over show menu with winner			///DUMMY DATA FOR NOW
				((StateGameOverMenu) _currentState).setGameWinner(gamewinner);
				((StateGameOverMenu) _currentState).setGameWinner( Player.PLAYER_ONE );
			}

...
public class StateHowto extends State {
	...
	public StateHowto (Taicho freegemas) {
		super(freegemas);
		...
		// Load font resource
		_fontLoading = Taicho.getPlatformResolver().loadFont("data/loadingFont.fnt", "data/normal.ttf", 70);
		...
		_loadingPos = new Vector2((Taicho.VIRTUAL_WIDTH - bounds.width) / 2, (Taicho.VIRTUAL_HEIGHT - bounds.height) / 2);
	...
	public void load() {
		...
		// Load fonts
		_fontTitle = Taicho.getPlatformResolver().loadFont("data/menuFont.fnt", "data/menu.ttf", 60);
		_fontText = Taicho.getPlatformResolver().loadFont("data/helpFont.fnt", "data/normal.ttf", 37);
...
//		_selectSFX = assetManager.get("data/select.ogg", Sound.class);
		...
		_titlePos = new Vector2(315 + (Taicho.VIRTUAL_WIDTH - 400 - bounds.width) / 2, 55);
	...
	public void render () {
		...
		_fontText.drawWrapped(batch, _helpText, _helpPos.x, _helpPos.y, Taicho.VIRTUAL_WIDTH - 450);
	}
